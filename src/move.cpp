...
std::vector <int> empty_board( std::array<std::array<char, 3>, 3> board)
{  
...
{  
    ...
    while (true) 
    {
        if(std::cin.fail())
        {
        std::cout<<player.name<<", ce n'est pas un nombre, réessaye !  "<<std::endl;
        std::cin.clear();
        std::cin.ignore(255, '\n');
        }
...
{  
    std::vector<int> tab = empty_board(board);
    int n = rand()%tab.size();
    int pos = tab[n];
    int i = (pos-1)/3;
    int j= (pos-1)%3;
